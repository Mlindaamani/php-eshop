...
class User {
  ...
  {
    $stmt = $this->database->dbconnection()->prepare("SELECT * FROM users WHERE first_name = ?");
  ...
  {
    $stmt = $this->database->dbconnection()->prepare("SELECT first_name FROM users WHERE id = ?");
